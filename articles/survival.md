...
we ask for the quantity we actually wanted (i.e., survival at a horizon)
...
| [`dpm_aft()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md) | \\\log T = m(x) + W\\, \\W\\ a mixture of normals | a log time ratio | the error density |
...
### Accelerated Failure Time (`weibull_aft()`, `loglogistic_aft()`, `lognormal_aft()`)
...
nothing when a single normal was the right answer, which is the property
...
**A contrast in the predictor is a log time ratio for all four
accelerated failure time families**,
[`dpm_aft()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md)
included. That follows from the structure rather than from the error:
with \\\log T = \eta(x) + W\\ and \\W\\ independent of \\x\\, every
quantile of \\T\\, the mean of \\T\\, and the geometric mean of \\T\\
all scale by \\e^{\Delta\eta}\\, whatever shape \\W\\ has. So a one-unit
increase in the predictor multiplies survival time by \\e\\ at every
quantile of the curve, and that reading does not depend on the error
being symmetric.

**A contrast in
[`ph()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md)’s
predictor is a log hazard ratio**, which is a different quantity. A
covariate that raises the hazard shortens survival, so the two move in
opposite directions, and their magnitudes are related only through the
shape of the baseline.

[`weibull_aft()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md)
is the one family that supports both readings at once, and that is a
property of its *error* rather than of its structural part, which it
shares with the other three. The smallest extreme value error is the
only one that makes an accelerated failure time model a proportional
hazards model as well, so a Weibull fit with shape \\k = 1/\sigma\\
carries a log hazard ratio of \\-k\\\Delta\eta\\ alongside its log time
...
their error’s location differently. This matters only when the level is
read directly rather than compared.

| Family | The error’s location | \\e^{\eta}\\ is |
|----|----|----|
| [`lognormal_aft()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md) | mean zero, symmetric | the median of \\T\\, which is also its geometric mean |
| [`loglogistic_aft()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md) | mean zero, symmetric | the median of \\T\\, which is also its geometric mean |
| [`dpm_aft()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md) | mean zero, asymmetric in general | the geometric mean of \\T\\, that is \\\exp E\[\log T \mid x\]\\, which is not the median |
| [`weibull_aft()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md) | location zero, so mean \\-\gamma\sigma\\ | the Weibull scale, which is the 63.2nd percentile of \\T\\ whatever the shape |

So
[`weibull_aft()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md)
is the family whose level needs the most care, not
[`dpm_aft()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md).
...
None of the three is normally the question, which is usually about
survival at a horizon (e.g., the difference in one-year survival between
treated and untreated). That is a contrast in \\S(t \mid x)\\ at a fixed
...
by hand from the draws, and it is the way we recommend reporting a
...
*shape* of the time distribution built around it. The six truths are:
...
We take as the primary metric the RMSE of the fitted survival function
\\S(t \mid x)\\ against the true one, on held-out subjects and over
twenty times spanning the 5th to 95th percentile of the event-time
distribution, because it is on the same scale for every family, which
the predictors are not.
...
The aggregate numbers hide *how* the misspecified fits are wrong. Below
are the fitted survival curves for three held-out subjects under the two
truths where the families disagree most, with the truth in the first
column.
...
The practical reading: **if the only question is who is at higher risk,
and the covariate effect does not move with time, the choice of family
barely matters.** It matters when the curve, an absolute probability, or
the shape of the hazard is needed, and it matters enormously when the
effect does move with time, which is the one case where the ordering is
not recoverable at all.

### The Recovered Error Density (`error_density()`)
...
time (the accelerated failure time families by stretching the time axis,
[`ph()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md)
...
accrues or wears off, a risk factor that matters only early). Not as a
hedge: under proportional hazards it is the most expensive option in the
...
contribution to the likelihood (i.e., a density for an event and
\\S(t)\\ for a censored observation). **The density is not on the same
scale for every family.** The accelerated failure time families,
[`dpm_aft()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md)
...
The way to avoid thinking about it at all is to compare on \\S(t \mid
...
[`ph()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md)
exposes `num_bins`, and it is fair to ask whether that amounts to a
choice the analyst is made to make. We swept it over a sixty-fold range
on the same two proportional-hazards truths, at 700 observations where
the default is 9:
...
sample size and more than half the Pareto-\\k\\ values went above .7, at
which point [`loo()`](https://mc-stan.org/loo/reference/loo.html) and
...
[`lognormal_aft()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md)
only when many models are being fit rather than one.

## Choosing a Family

| If | Use |
|:---|:---|
| there is no view about the shape of anything | [`dpm_aft()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md) |
| the same, but the fit has to be quick | [`lognormal_aft()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md) or [`loglogistic_aft()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md) |
| the shape of the baseline hazard is the point | [`ph()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md) |
| a hazard ratio is wanted from a parametric fit | [`weibull_aft()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md) |
| only the ordering of risk is needed | any of them, if the effect is constant in time |
| the covariate effect may move with time | the discrete-time route, not a family |
| a treatment effect is to be reported | any of them, through `type = "survival"` |
...
which arm does better), we have a great deal of freedom in the choice
and should spend our effort elsewhere. If the question is an absolute
