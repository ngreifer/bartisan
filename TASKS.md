...
- **`.git/` does not carry the `com.dropbox.ignored` xattr.** It holds only the init skeleton (14 files, no commits). Rebuilding it in the canonical order was declined, so it is untouched and **nothing has been committed**. Decide before the first commit — see `~/.config/agents/AGENTS.md` § "Dropbox Sync Exclusions". At 14 files the one-time sync flood would be trivial, but the directory stays a latent problem for any later `git gc`.
- `README.md` is generated from `README.Rmd` with `knitr::knit()`, and needs re-knitting whenever the Rmd changes.
