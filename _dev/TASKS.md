...
- [ ] A **formal variable-selection test** rather than a threshold on `prop_used`, which `dbarts`, `SoftBart` and `bartMachine` all have and this does not. `variable_importance(draws = TRUE)` now returns the per-draw counts, so a caller can compute the posterior probability that one predictor takes more rules than another and does not have to read a difference off the table; what is missing is a calibrated null to test against, which is what the permutation approach in those packages supplies. The partial-dependence half of this item is closed: *marginaleffects*' `plot_predictions()` draws it, and `plot()` on the output of `variable_importance()` covers the usage ranking.
- [ ] **A comparison between a family with a point mass and one without.** `loo()` cannot compare `tweedie()` with `gaussian()` or `dpm()` on an outcome with exact zeros, since at a zero one reports a probability and the other a density, and `vignette("comparison")` dropped its section on this case on 2026-09-24 rather than send users to hand-written code. The candidate is leave-one-out CRPS or scaled CRPS: proper scores computed from predictive draws, so a point mass is no obstacle, and *loo* 2.10 already has `loo_crps()` and `loo_scrps()` (two independent sets of `posterior_predict()` draws plus `log_lik()` for the weights). What is missing is methods for bartisan fits and a comparison function, since `loo_compare()` refuses anything that is not class `loo`. `kfold()` with a score argument would avoid the approximation but is too slow for routine use and does not by itself give a criterion. The old section's text, including the posterior predictive check on the share of zeros, is in the history of `vignettes/comparison.Rmd`.
...
- The effect forest gets no propensity score. Linero's correction for the BCF-type model adds a function of the score to the effect as well as the control function, since the ATE's confounding bias has a term from the covariance of the effect with the score. Test with a design where heterogeneity follows the propensity score before offering it.
- **Done 2026-09-25.** `bcf()` refused `propensity = TRUE` for a continuous treatment on the grounds that the balancing score is a density. Linero's correction for a continuous exposure (linear case) uses the fitted conditional mean of the exposure, and `bcf()` now fits that with `gaussian()` and adds it to the control function as it does a binary treatment's probability.
- A prior check on the scale of the confounding bias: `prior_only` makes its prior computable, and Oganisian and Linero compare its posterior with a Horvitz-Thompson estimate to expose a dogmatic prior.
- **Done 2026-09-25.** `vignette("causal")`'s single-forest model now includes the propensity score estimated earlier in the vignette, with Linero (2024) as the reason. The ATE moved from 0.0616 [0.0119, 0.115] to 0.0638 [0.0074, 0.120].

## Overlap and the width of the CATE interval (2026-09-25)

Li, Ding and Mealli (2023, Example 4.1) show a BART interval for tau(x) keeping its width where the treatment groups do not overlap, and suggest soft decision trees as the mitigation. Checked with `_dev/overlap-uncertainty.R` on their design, ten replications. Their finding reproduces for their own design, a separate BART per arm, and soft rules do **not** fix it there: the posterior sd barely moves (0.39 in the overlap, 0.52 and 0.68 in the tails, soft rules), with bias of 5 to 6 and coverage 0 in both tails, because each arm extrapolates flat past its own data. The single-forest designs bartisan fits do widen, by four to nine times in the tails (soft: 0.26 against 1.48 and 1.08; `bcf()`: 0.31 against 2.86 and 2.09), and their intervals cover there; soft rules are also nearly unbiased in the tails (under 0.1), against 0.5 to 0.6 for hard rules and 1.4 to 1.9 for `bcf()`. One simple design, linear in x with a constant effect, which suits a model that shares structure across the arms. Bore on the sentence in `vignette("causal")` that said the interval would not reveal extrapolation, which was rewritten on 2026-09-25 to report this result, and makes an overlap diagnostic built on posterior uncertainty (as in *bartCause*'s common-support rules) plausible for bartisan's designs.

## The ATT with the observed treated mean (2026-09-25, investigated, nothing changed)

Asked: can a BCF model reproduce the treated group's observed mean exactly for the ATT, as a GLM does? **Not as a property of the model.** In a GLM the equality comes from the maximum likelihood score equations of a canonical link: on lalonde, a fully interacted `gaussian()` or `quasipoisson()` fit matches the treated mean to 1e-11, while `statmod::tweedie(var.power = 1.5, link.power = 0)` misses by 2.30, the log link not being canonical for that variance. A posterior solves no score equations, and a restriction forcing the treated mean to equal the observed one in every draw would be treating the treated outcomes as known, which is a change of estimand rather than of model. The parameterization with an empty control function and a coefficient forest centered on the treated comes closest (the treated mean is one constant, whose posterior concentrates near the observed mean), but it is still not exact, and it makes the treated units' conditional effects meaningless, since their fitted outcome no longer varies with the covariates.

**Exact at the level of the estimand.** Using each treated unit's observed outcome and imputing only the untreated one gives the finite-sample SATT, which is what *bartCause*'s `"ite"` extraction type builds, and what its `summary(target = "sate")` reports: its code (1.0.10) forms the SATE point estimate as observed outcome minus expected counterfactual, so for the ATT it is exactly the observed treated mean minus the average untreated prediction. Its help page says the three targets share one point estimate; for the SATE that is not what the code does (checked on 2026-09-25 with `_dev/bartcause-targets.R`). A Bayesian bootstrap over the treated units keeps the treated mean exact in expectation and restores its sampling uncertainty, which is the closest analogue of what a GLM reports.

**Measured** with `_dev/att-observed-mean.R`: the model's treated mean was 6373 against an observed 6349, well inside its own uncertainty, and the ATT moved by at most 65 across the four versions. Widths were 3441 model-based, 2590 with the observed mean and expected `Y(0)`, 3086 with predicted `Y(0)`, and 3266 under the Bayesian bootstrap. The expected-`Y(0)` version is narrowest because it drops both the treated mean's sampling variability and the residual noise of the missing outcome, so it is too narrow for either the sample or the population reading. Left as a possible `estimate_effect()` option for sample estimands across ATE, ATT and ATC, not for CATEs.

## Prior-only fits ran on the slow path (2026-09-24)

`prior_only = TRUE` gives every observation a weight of zero, and every augmentation refuses that: the logit one needs positive weights and the rest need weights of exactly one. So a prior-only fit ran on the general family, which evaluated each observation's log density and derivatives at every step and then multiplied them by zero. Profiled with `sample`: the posterior fit spent its time in `LogitAugmentedFamily`'s accumulation loops, the prior-only fit in `BinomialFamily::score_info_unit()`, `logdens_unit()`, `log1p()` and `exp()`, with forests of the same size (about 75 rules per draw) and similar leaf scales. In `vignette("diagnostics")` the one-chain prior-only fit took 10.7 seconds against about 3 per chain for the posterior.

**Fixed in `src/family.h`** by skipping a zero-weight observation in the `Concrete` loops (`accumulate1_block`, `accumulate1_fused`, `accumulate2_loop`, `logdens_block`) and in the base class's `logdens()`, `dlogdens()`, `info()` and `score_info()`. The R-backed families still evaluate a whole block in one call to R; they were left alone.

**Measured** with `_dev/zero-weight-skip.R`: nine configurations, five interleaved repetitions per build, each build in its own process. Every configuration's draws are bit-identical before and after, prior-only fits included. Prior-only logit went from 10.89 to 2.66 seconds with soft rules and from 6.35 to 1.36 with hard rules, and prior-only Poisson from 3.77 to 1.93. The soft prior-only fit now costs about what the posterior does (2.50); the hard one is still about 1.5 times the posterior (0.91), which is the rest of the general path and was not chased. Ordinary fits moved by -2.3% to +1.3%, except `poisson()` at +5.2%, slower in four of five pairs against a run-to-run spread of about 11%; whether that is real was not settled by five pairs.
