...
like [`glm()`](https://rdrr.io/r/stats/glm.html).
...
|----|----|----|
| [`gaussian()`](https://rdrr.io/r/stats/family.html) | identity | residual standard deviation |
| [`binomial()`](https://rdrr.io/r/stats/family.html) | logit, probit, cloglog | — |
| [`poisson()`](https://rdrr.io/r/stats/family.html) | log | — |
| [`negbin()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md) | log | dispersion |
| `Gamma("log")` | log | shape |
| [`ordinal()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md) | logit, probit, cloglog | cutpoints |
| [`multinomial()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md) | logit, probit | latent covariance, for probit |
| [`weibull_aft()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md), [`loglogistic_aft()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md), [`lognormal_aft()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md) | — | scale |
| [`ph()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md) | — | baseline hazard per bin |
| [`location_scale()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md) | identity | — |
| [`zi_poisson()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md) | log | — |
| [`zi_negbin()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md) | log | dispersion |
| [`Beta()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md) | logit, probit, cloglog | precision |
| [`ordbeta()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md) | logit | 2 cutpoints, precision |
| [`dpm()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md) | identity | error mixture, concentration |
...
a logical or a two-level factor
[`binomial()`](https://rdrr.io/r/stats/family.html), a factor with more
levels
[`multinomial()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md),
and any other numeric response
[`dpm()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md),
whatever its number of distinct values. A message says which was chosen;
naming `family` silences it. A count is deliberately *not* read as
[`poisson()`](https://rdrr.io/r/stats/family.html), and neither is a
numeric response whose two values are not zero and one: in both cases
the guess would be a modeling decision rather than a reading of the
response’s type.

Ordinary [`stats::family`](https://rdrr.io/r/stats/family.html) objects
work unchanged, so moving a model from
[`glm()`](https://rdrr.io/r/stats/glm.html) to
[`bartisan()`](https://ngreifer.github.io/bartisan/reference/bartisan.md)
is a one-word change.
[`multinomial()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md),
[`location_scale()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md)
and the two zero-inflated families fit one forest per unconstrained
parameter, so the excess-zero mechanism of a zero-inflated count model
is itself nonparametric rather than a single constant.
[`ordbeta()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md)
is the ordered beta regression of Kubinec (2023), for a proportion or
slider response with mass piled up at zero and one.
[`multinomial()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md)
follows Murray (2021) in fitting one forest per category and leaving the
model unidentified, so that the prior does not depend on which category
was singled out as the reference.

[`dpm()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md)
is the default worth reaching for on a numeric response, and it is not a
distribution but a way of not choosing one. It is DPMBART (George et
al. 2019): the sum of trees for the mean, as
[`gaussian()`](https://rdrr.io/r/stats/family.html) has, and a Dirichlet
process mixture of normals for the errors instead of one normal, so the
error distribution comes out as whatever shape the data ask for.
[`error_density()`](https://ngreifer.github.io/bartisan/reference/error_density.md)
reports that shape with a pointwise interval. Against
[`gaussian()`](https://rdrr.io/r/stats/family.html) on held-out
predictive log score it is level when the errors are normal (−2097
against −2097), and ahead by 175 and 224 log points when they are heavy
tailed or skewed — on the skewed case it also cuts the interval width
for the regression function from 1.59 to 0.99 while holding coverage. It
is *not* the family for heteroskedasticity: a mixture makes the error
distribution flexible but keeps it the same at every `x`, which is what
[`location_scale()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md)
is for, and which wins that case by 119 log points. Conditional on the
mixture the target is still quadratic, so a fit costs 1.3× a Gaussian
one against
[`location_scale()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md)’s
9×. It also does not pay for its flexibility: on normal errors, where
...
[`gaussian()`](https://rdrr.io/r/stats/family.html) is that it takes
prior weights, which
[`dpm()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md)
refuses. The additive predictor is the conditional mean for both:
nothing forces the mixture to be centered, so the sampler works in a
chart where only their sum is identified and reports in the one where
the mixture has mean zero.
...
goes through every family: what each is and is not for, how to choose
among the links a family offers, how to choose among the zero-inflated
models, and what a family inferred from the response will be if you name
none.
[`vignette("survival")`](https://ngreifer.github.io/bartisan/articles/survival.md)
takes the five censored-response families on their own: what each one is
estimating, which shapes of hazard each can and cannot represent, how
they compare across six data-generating truths and across censoring from
none to 70%, and what to do when hazards are not proportional.
...
and neither does a link you write yourself and pass the way
[`glm()`](https://rdrr.io/r/stats/glm.html) takes one.
...
logistic one and equally accurate; `gate` in
...
[`bartisan_control()`](https://ngreifer.github.io/bartisan/reference/bartisan_control.md)
groups its arguments so that the ones that matter come first. Four are
worth a look before any fit.
...
fit, and that is worth using.
[`location_scale()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md)
spends about 90% of its time on the scale forest, and a variance surface
needs less capacity than a mean surface, so a smaller second forest cuts
the fit from ten times a Gaussian one to three and a half at the same
accuracy.
[`?bartisan_control`](https://ngreifer.github.io/bartisan/reference/bartisan_control.md)
has the measurements, along with the tree-count curves for the two kinds
of rule — soft rules peak at around 20 trees and get worse past that,
hard rules keep improving to 200.
...
Every draw of every tree is retained, so
[`predict()`](https://rdrr.io/r/stats/predict.html) can return the full
...
A [`vc()`](https://ngreifer.github.io/bartisan/reference/vc.md) term
makes a predictor’s coefficient a forest of its own, so the model is a
control function plus one function-valued coefficient per marked
predictor:
...
covariate. [`coef()`](https://rdrr.io/r/stats/coef.html) returns the
coefficient at each observation, which is what a coefficient becomes
when it varies, and every **marginaleffects** estimand works unchanged
because the combination happens inside the model.
...
functions fitted at once, and
[`coef()`](https://rdrr.io/r/stats/coef.html) returns both. No other
package fits a varying coefficient on a distributional parameter other
than the mean.
...
[`bartisan()`](https://ngreifer.github.io/bartisan/reference/bartisan.md)
warns. The effect is linear in the covariate unless the coefficient’s
forest may split on it, which `vc(z, ~ z + x1)` allows and which is how
a dose response becomes a curve. And a factor gets one forest per level,
coded symmetrically rather than against a reference;
`center = "estimate"` draws the coding instead, which is free for a
binary covariate and a real restriction above two levels.
...
One caveat, documented in `?bartisan-marginaleffects`.
[`avg_slopes()`](https://rdrr.io/pkg/marginaleffects/man/slopes.html) is
...
[`simulate()`](https://rdrr.io/r/stats/simulate.html) behave as they do
for a `glm`, and `insight` reads the fit through them, so the
*easystats* packages see it too. `?bartisan-interop` documents the scale
each replicate comes back on — a binomial replicate is a proportion, a
categorical one a category index, an accelerated-failure-time one an
uncensored event time — and why a leave-one-out estimate is strained for
a model as flexible as a forest.
...
reports the average number of splitting rules using each predictor and
the proportion of draws using it at all. The indicator columns of a
factor share one weight, so a factor is selected as a whole rather than
one level at a time.
...
`default_family()`, which returns
[`dpm()`](https://ngreifer.github.io/bartisan/reference/bartisan-families.md)
– a Dirichlet process mixture for the error distribution, not a Gaussian
– so those rows timed a strictly more expensive model than the three
packages they were compared against, and their accuracy column described
a different fit. Naming the family is worth about 35%.
...
stopped. On this task that is 0.28 s against 0.47 s for `dbarts` – so
the comparison charged `dbarts` for two thirds again as much work as
...
derivative against two – rather than speed. Fixing the bandwidth
outright is the fix for the first, and is *not* the default, because on
a function with jumps it more than doubles the error – letting the rules
sharpen towards hard ones is what the move is for.
...
is already exponential in the sense above, censoring included – which
...
files for a later `R CMD INSTALL` to reuse.
[`bartisan()`](https://ngreifer.github.io/bartisan/reference/bartisan.md)
warns once per session when it detects one;
...
the problem, and the advice says so instead of listing everything a
reader might try. And it leaves the leaf scale out, because it mixes
badly in every implementation – 1.12 in *dbarts* and 1.16 in *stochtree*
against 1.19 here, on the same data – and its disagreement never reaches
the fitted function.
...
probit, a normal with a Pólya-Gamma precision for the logit, an
...
binary response is the same construction with one threshold. The
complementary log-log error enters the two families with opposite signs,
and `?predict.bartisan` says why.
...
move. It is controlled by `sigma_mu_ramp` in
[`bartisan_control()`](https://ngreifer.github.io/bartisan/reference/bartisan_control.md).
...
median; `update_sigma_mu = FALSE` pins it.
...
costs three unless you supply the derivatives. Both are usable at the
