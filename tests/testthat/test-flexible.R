...
test_that("blocked evaluation reproduces the per-observation path", {
  ...
  for (nm in names(responses)) {
    ...
    set.seed(5)
    plain <- genbart(y ~ ., data = dd, family = responses[[nm]][["family"]],
                     control = quick_control(num_burn = 0L, num_save = 20L))
    set.seed(5)
    blocked <- genbart(y ~ ., data = dd, family = responses[[nm]][["family"]],
                       control = quick_control(num_burn = 0L, num_save = 20L,
                                               block_eval = TRUE))
...
test_that("a composed link gives the derivatives its closed form does", {
  ...
  for (blocked in c(FALSE, TRUE)) {
    got <- .genbart_derivs(y, rep(1, n), list(eta), "binomial", "logit", opts,
  ...
  got <- .genbart_logdens(y, rep(1, n), list(eta), "binomial", "logit", opts,
                          matrix(0, 3L, 0L))
...
test_that("a link with no derivative falls back on differences", {
  ...
  for (blocked in c(FALSE, TRUE)) {
    with_deriv <- .genbart_derivs(y, rep(1, n), list(eta), "binomial", "logit",
                                  cauchit_opts(TRUE), matrix(0, 2L, 0L), 0L,
                                  FALSE, blocked)
    without <- .genbart_derivs(y, rep(1, n), list(eta), "binomial", "logit",
...
test_that("stats family objects carry an uncompiled link through a fit", {
  ...
  fit <- genbart(y ~ ., data = d, family = binomial("cauchit"),
  ...
  fit <- genbart(y ~ ., data = d, family = binomial(invented),
                 control = quick_control())
...
test_that("a nuisance parameter is still drawn under a composed link", {
  ...
  fit <- genbart(y ~ ., data = d, family = gaussian("log"),
                 control = quick_control())
...
test_that("custom_family reproduces the family it imitates", {
  ...
  compiled <- .genbart_logdens(y, rep(1, n), list(eta), "poisson", "log",
                               list(), matrix(0, 4L, 0L))
  supplied <- .genbart_logdens(y, rep(1, n), list(eta), "custom", "identity",
                               opts, matrix(0, 4L, 0L))
  ...
  a <- .genbart_derivs(y, rep(1, n), list(eta), "poisson", "log", list(),
                       matrix(0, 4L, 0L), 0L, FALSE, FALSE)
  b <- .genbart_derivs(y, rep(1, n), list(eta), "custom", "identity", opts,
...
test_that("supplied derivatives are used and must be the right shape", {
  ...
  expect_error(
    .genbart_logdens(y, rep(1, n), list(eta), "custom", "identity",
...
test_that("a broken log density errors cleanly and leaves nothing behind", {
  ...
  for (i in 1:3) {
    expect_error(genbart(y ~ ., data = d, family = broken,
                         control = quick_control()),
                 "deliberate failure")
  }
...
test_that("supplied derivatives are used by both routes alike", {
  ...
  per_unit <- .genbart_derivs(y, rep(1, n), list(eta), "custom", "identity",
                              opts, matrix(0, 3L, 0L), 0L, FALSE, FALSE)
  blocked <- .genbart_derivs(y, rep(1, n), list(eta), "custom", "identity",
...
test_that("a separable response is flagged rather than passed off silently", {
  ...
  expect_warning(genbart(y ~ ., data = d, family = binomial(),
  ...
  expect_no_warning(genbart(y ~ ., data = d, family = binomial(),
                            control = quick_control(num_burn = 200L,
                                                    num_save = 200L,
...
test_that("a fit holding R closures survives a round trip through a file", {
  ...
  d$y <- stats::rbinom(100, 1, stats::pcauchy(truth))
  linked <- genbart(y ~ ., data = d, family = binomial("cauchit"),
                    control = quick_control())

  d2 <- d
  d2$y <- stats::rpois(100, exp(truth + 2))
  supplied <- genbart(y ~ ., data = d2,
                      family = custom_family(
                        function(y, eta) y * eta[, 1L] - exp(eta[, 1L]),
                        start = log(mean(d2$y))),
                      control = quick_control())
