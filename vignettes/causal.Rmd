...
score as a covariate, which is recommended by @carnegie2019 to slightly
improve performance (in this case it doesn't affect the result).
Normally, we would examine convergence diagnostics for this model to
make sure it was fit correctly; see `vignette("diagnostics")` for more
...
[^1]: Here we report the risk difference, the default quantity returned
    by `avg_comparisons()` with a binary outcome. You can also set
...
coefficient BART model, the BCF. By default, `bcf()` estimate a
propensity score using a logistic BART model and includes that as a
covariate in the control function, as recommended by @hahn2020.
