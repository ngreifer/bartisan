...
confounders) in a flexible manner [@hill2011]. BART has repeatedly been
shown to outperform other effect estimation methods in competitions
[@dorie2019]. In this vignette, we demonstrate how to use BART
implemented in *bartisan* to estimate treatment effects. This includes
both standard BART as well as Bayesian causal forests (BCF), which is an
improvement on traditional BART that works by fitting separate BART
models for the outcome under control and for the treatment effect itself
as a varying coefficient model.
...
were also more likely to die. We can use `cobalt::bal.tab()` to see how
the patients differ between groups:
...
ps_fit <- bartisan(
  ...
  data = rhc, family = binomial("probit"), chains = 4
)

prop_score <- fitted(ps_fit)
```

We can use `cobalt::bal.plot()` to examine the overlap of the propensity
score distribution between the groups:
...
$\tau_{\text{ATE}} = E[Y(1)] - E[Y(0)]$, which is a function of the
unobserved potential outcomes $Y(1)$ and $Y(0)$, we can use the
assumptions above to express it as a function of observed quantities:

$$E[Y(1)] - E[Y(0)] = E \left[ E[Y | X, A = 1] \right] - E \left[ E[Y | X, A = 0] \right]$$
To estimate $E \left[ E[Y | X, A = a] \right] = \theta_a$, we use
g-computation [@snowdenImplementationGComputationSimulated2011]:

$$\hat{\theta}_a = \frac{1}{n}\sum_{i=1}^n {\mu(a, x_i)}$$

where $\mu(a, x_i)$ is the predicted value from a regression of $Y$ on
$A$ and $X$ for a unit with covariate profile $X=x_i$ and treatment $A$
set to $a$. This estimator is sometimes also known as the "regression
estimator" or the "plug-in estimator".

Here, we use traditional BART and BCF to model $\mu(A, X)$. The rest of
the analysis comes from the definitions above.
...
read off it. `?bartisan_control` has the measurements behind both halves
of this, and `split_prior` is the alternative when there are enough
covariates that weighting them all alike is wasteful.
...
`vignette("diagnostics")` for more information on how to do that.
...
catheterized, and if none were. `marginaleffects::avg_predictions()`
...
The two rows are the estimates of $E[Y(0)]$ and $E[Y(1)]$, each averaged
over the observed covariate distribution. Reporting both is often more
informative than reporting their difference alone, because a difference
of six percentage points means something different against a baseline of
63% than it would against 5%.
...
catheterization raises the probability of death by about
`r round(100 * abs(ate$estimate[1]))` percentage points, with an
interval running from roughly `r round(100 * abs(ate$conf.low[1]))` to
`r round(100 * abs(ate$conf.high[1]))`.
...
@hahn2020 identified this mechanism. Their remedy is to give the
treatment effect its own forest with its own prior, so that shrinking
the confounding part does not shrink the effect, which is the Bayesian
causal forest (BCF) model, a special case of the varying coefficients
BART model. `bcf()` fits it.

One specifies the control function in the model formula and identifies
the treatment in the `treatment` argument. `bcf()` then fits a varying
coefficient BART model, the BCF.
...
Using `bcf()` rather than writing the varying coefficient BART model by
hand sets four settings to improve effect estimation: the effect gets a
forest of its own, the estimated propensity score goes into the control
function and not into the effect forest, the effect forest gets fewer
trees because effect heterogeneity is usually simpler than a prognostic
surface, and a binary treatment's coding is drawn rather than fixed, so
the answer does not depend on which arm was written as 1. Note that
`sparsity = FALSE` is not among them: the reason for it in the section
above is that the prior can drop the predictor whose contrast we want,
and here the treatment is the coefficient rather than a predictor the
forest splits on, so nothing can drop it.

The output is just like a regular BART model, so we can use
`avg_comparisons()` to extract the average treatment effect estimate.
...
one to report. `coef()` is for looking at how the effect varies from
patient to patient, not for reporting its size.
...
would have done. See `vignette("families")`.
...
estimate and its standard error, which `avg_comparisons()` provides.

## Where to go next

`vignette("effects")` covers the estimand machinery in more detail,
including subgroup effects and testing whether they differ.
`vignette("diagnostics")` covers checking the fit, which should happen
